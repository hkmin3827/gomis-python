...
def _is_thumbs_up(lm) -> bool:
    ...
    Method B — TIP~MCP AND TIP~PIP 이중 조건으로 '진짜 접힘' 판별.
    손 측면 각도 ±20° 이내 자연 허용 (거리 기반, 방향 무관).

    임계값 근거 (MediaPipe 정규화 좌표, ref = WRIST~MIDDLE_MCP):
      편 손가락 : TIP~MCP ≈ ref*1.0+,  TIP~PIP ≈ ref*0.65+
      접은 손가락: TIP~MCP ≈ ref*0.50~0.80, TIP~PIP ≈ ref*0.25~0.50
    → MCP 임계 0.85 + PIP 임계 0.55 이중 충족 시에만 '접힘' 판정.
    """
    ref = _palm_ref(lm)
    if ref < 1e-6:
        return False

    # 엄지: 손목에서 충분히 멀리 뻗어 있음
    thumb_extended = _lm_dist(lm, THUMB_TIP, WRIST) > ref * 1.2

    # 나머지 4손가락: TIP~MCP 가깝고(1차) AND TIP~PIP도 가까워야(2차) 진짜 접힘
    # 손바닥 펴기 시 TIP~PIP ≈ ref*0.65+ 이므로 0.55 임계에서 걸러짐
    def _finger_curled(tip: int, pip: int, mcp: int) -> bool:
        return (
            _lm_dist(lm, tip, mcp) < ref * 0.85 and
            _lm_dist(lm, tip, pip) < ref * 0.55
        )

    index_curled  = _finger_curled(INDEX_TIP,  INDEX_PIP,  INDEX_MCP)
    middle_curled = _finger_curled(MIDDLE_TIP, MIDDLE_PIP, MIDDLE_MCP)
    ring_curled   = _finger_curled(RING_TIP,   RING_PIP,   RING_MCP)
    pinky_curled  = _finger_curled(PINKY_TIP,  PINKY_PIP,  PINKY_MCP)
